...
class IconType(object):
    """ Configuration values for visualization icons. """
...
class Icon(object):
    """ Configuration objects for visualization icons. """
...
class Board(object):
    """ This class managed a Tkinter visualization. Visualization must be run
    in the main thread, so any logic that interacts with it must run on a
    background thread. """
    def __init__(self, pixWidth=900, pixHeight=900):
        """ Initialized with width/height, sets up main canvas. """
        self.root = tk.Tk()
    ...
    def generate(self, width, height, icons=[]):
        """ Lays out a grid of proportionally spaced rectangles, where width and height
        match number of horizontal and vertical rectangles. Also draws the icons
    ...
    def startDisplay(self):
        """ Presents visual display. """
        self.root.mainloop()
        for boardIcon in self.boardIcons:
            self.board.delete(boardIcon)
        self.boardIcons = []
    def stopDisplay(self):
        """ Removes visual display. """
        self.root.quit()
